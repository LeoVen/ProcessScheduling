...
Status file_load(DynamicArray *process_table)
{
	...
	while (text->len > 1)
	{
		...
		// Convert pid
		sscanf(pid_s->buffer, "%lu", &pid);

		// Convert cpu
		sscanf(cpu_s->buffer, "%lu", &cpu);

		// Convert io
		sscanf(io_s->buffer, "%lu", &io);

		// Convert priority
		sscanf(pri_s->buffer, "%lu", &pri);
